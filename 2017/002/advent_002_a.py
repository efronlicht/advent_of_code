...
input_raw = """5806	6444	1281	38	267	1835	223	4912	5995	230	4395	2986	6048	4719	216	1201
...
def parse_rows(raw: str) -> T.Iterable[T.Tuple[int, ...]]:
    for row in raw.split("\n"):
        yield tuple(int(x) for x in row.split())

def row_diff(row: T.Tuple[int, ...]) -> int:
    return abs(max(row)-min(row))

def checksum(raw: str) -> int:
    return sum(row_diff(row) for row in parse_rows(raw))

if __name__ == '__main__':
    print(checksum(input_raw))