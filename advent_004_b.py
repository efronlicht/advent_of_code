...
raw_input = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
def validate(passphrase: T.Iterable[str]) -> bool:
    seen: T.Set[str] = set()
...
def passphrases(raw: str) -> T.Iterable[T.List[str]]:
    phrases = raw.split("\n")
    for phrase in raw.split("\n"):
        yield phrase.split()

if __name__ == '__main__':
    print(sum(validate(x) for x in passphrases(raw_input)))